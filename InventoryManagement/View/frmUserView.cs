...
namespace InventoryManagement.View
{
    public partial class frmUserView : SampleView
    {
        ...
        private void LoadDt()
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                if (int.TryParse(roleValue, out role))
                {
                    ...
                    guna2DataGridView1.Rows.Add(count++, row["IDUSER"], row["USERNAME"], row["FULLNAME"], row["PASSWD"], row["MACTY"], row["MADVI"], roleName, row["Email"]);
                }
                else
                {
                    // Nếu không thể chuyển đổi, sử dụng "Unknown"
                    guna2DataGridView1.Rows.Add(count++, row["IDUSER"], row["USERNAME"], row["FULLNAME"], row["PASSWD"], row["MACTY"], row["MADVI"], "Unknown", row["Email"]);
                }
            }
        }
        ...
        private void guna2DataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            int selectedUserId = Convert.ToInt32(guna2DataGridView1.CurrentRow.Cells["dgvid"].Value); //dgvid=IDUSER
            ...
            if (colName == "dgvEdit")
            {
                ...
                frm.cbbMaCty.SelectedValue = guna2DataGridView1.CurrentRow.Cells["dgvCty"].Value.ToString();
                frm.LoadDonViByCongTy(); // Tải lại đơn vị
                frm.cbbMaDvi.SelectedValue = guna2DataGridView1.CurrentRow.Cells["dgvdvi"].Value.ToString();

                frm.cbbRole.Text = guna2DataGridView1.CurrentRow.Cells["dgvRole"].Value.ToString();
